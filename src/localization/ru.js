export const ruLocalization = {
    translation: {
        menu: {
            t_blog: "Блог",
            t_projects: "Проекты",
            t_utils: "Утилиты",
        },
        footer: {
            t_questions: "У тебя остались вопросы?",
            t_say: "Не стесняйся напиши!",
            t_name: "Виталий Зарубин",
        },
        index: {
            // BlockMain
            t_main_hello: "Бонжур",
            t_main_subtitle: "Я программист, а это мой сайт",
            t_main_email_label: "Емейл",
            t_main_btn: "Лан, давай",
            t_main_info: "* Получите мое резюме прямо в свой почтовый ящик",
            t_main_error: "Пожалуйста, заполните почту, она нужна для отправки",
            t_main_success: "Спасибо за Ваш интерес! Будет следано",
            // BlockAbout
            t_about_title: "Обо мне",
            t_about_me: "Android. Web Full-Stack" +
                "\nИ чуток iOS & PC",
            t_about_text: "Я работаю программистом 10 лет. Начинал программистом на PHP, писал фичи для Webasyst (CMS для магазинов). " +
                "\n\nПоняв что CMS дает мало свободы (на самом деле сменил работу) занялся framework-ом yii и после yii2. Очень ждал yii3 но не дождался и освоил React как замену." +
                "\n\nГоворили что Android программисты получают больше. Я решил проверить. Так я и стал лидом Android. Но прошлое не забываю и пишу частенько web фичи." +
                "\n\nА еще я считаю что программист это тот кто может писать все. Об этом так же скажет Вам мой GitHub. Чего там только нет, любопытсво мое обширно.",
            t_about_follow: "Преследовать",
            // BlockProjects
            t_projects_android_title: "Android",
            t_projects_android_text: "На данный момент сейчас проффисионально занимаюсь Android разработкой. Ну и не проффисионально тоже.",
            t_projects_android_btn: "Заценить",
            t_projects_ios_title: "iOS",
            t_projects_ios_text: "Swift очень похож на Kotlin, а SwiftUI на Jetpack Compose так что я считаю что я вполне могу клепать iOS.",
            t_projects_ios_btn: "Заценить",
            t_projects_web_title: "Web",
            t_projects_web_text: "Когда-то проекты на web кормили меня. Сейчас я пишу (как например этот сайт) для души и по кайфу.",
            t_projects_web_btn: "Заценить",
            t_projects_pc_title: "PC",
            t_projects_pc_text: "Люблю я Linux и всю эту тему про свободу. По мере возможностей пишу Open Source для людей.",
            t_projects_pc_btn: "Заценить",
            // BlockResume
            t_resume_title: "Мой путь",
            t_resume_role_android: "Android Developer",
            t_resume_role_web: "Web Full-Stack",
            t_resume_surf_text: "Сейчас работаю в этой копании. Самой лучшей, любимой, идеальной, продуктивной. Ну а что еще я могу сказать что бы меня не уволили?",
            t_resume_surf_date: "Авг 2021 - И сейчас",
            t_resume_clowder_text: "Здесь я провел большую часть своей жизни как мне кажется. Прошел огонь и воду написал сотни сайтов и несколько андройд приложений. Научился писать все что нужно и не смотреть на какие-то там языки, которые можно освоить за недельку.",
            t_resume_clowder_date1: "Авг 2015 - Авг 2021 · 6 г 1 м",
            t_resume_clowder_date2: "Сен 2014 - Авг 2015 · 1 г",
            t_resume_privezem_text: "Мы писали сайты по продаже товаров с доставкой, когда это еще не было мейнстримом. Там я освоил yii, js, css, но как казалось начальству не рос. Видели бы они сейчас меня и мой живот.",
            t_resume_privezem_date: "Нов 2013 - Сен 2014 · 11 м",
            t_resume_agamaya_text: "Моя первая работа. После кусов php и самостоятельнго изучения Qt я прошел их жесточайший отбор, за что спасибо им. Тут мне рассказали что такое Git и показали что за компом можно иметь не меньше чем на заточке пил.",
            t_resume_agamaya_date: "Май 2013 - Ноя 2013 · 7 м",
            // BlockSkills
            t_skills_title: "Скилы",
            t_skills_android_title: "Android",
            t_skills_android_text: "Более 7 лет работю андройд программистом. Помню те времена когда еще MVVM не были популярным и MVP вполне всех устраивал.",
            t_skills_ios_title: "iOS",
            t_skills_ios_text: "Опыта не много, но он есть. Изучаю swiftUI. На досуге пишу KMM проект. Рзаработка очень схожа с jetpack compose на котором пишу уже год.",
            t_skills_markup_title: "Markup",
            t_skills_markup_text: "Около 5 лет клепал сайты как пулемет. Сейчас только для себя и open source. Например этот я сделал сам, так же как и его дизайн.",
            t_skills_db_title: "Database",
            t_skills_db_text: "Моя любимая база с которой я работал больше всего - MySql. Много работал как с реляционными базами так и no noSql.",
            t_skills_web_title: "Web",
            t_skills_web_text: "Много писал сайтов на Yii2. Сейчас плотно изучаю React, к сожалению похоже на то что yii загибаеться и больше не пригодиться.",
            t_skills_linux_title: "Linux",
            t_skills_linux_text: "Я из тех барадатых любителей linux. Долгое время собирал gentoo пока не надоело. Пишу приложения на разных языках open source.",
        }
    },
};